...
export default {
  ...
  created() {
    ...
    this.allocateDeviceParams.applyId = this.$route.query.deviceTypeId;
  ...
  methods: {
    ...
    allocateDevice(index, row) {
      this.allocateDeviceParams.ids.push(row.id)
      this.batchAllocateDevice(ids);
    },
    //删除
    ...
    handleBatchOperate() {
      ...
      alert(ids)
      this.allocateDeviceParams.ids = ids
