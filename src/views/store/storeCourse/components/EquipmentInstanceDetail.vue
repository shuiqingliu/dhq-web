<template>
  <el-card class="form-container" shadow="never">
    <el-form :model="equipmentInstance" :rules="rules" ref="equipmentInstanceForm" label-width="150px">
      ...
      <el-form-item label="生产日期：" prop="produceDate">
        <el-date-picker
      v-model="equipmentInstance.produceDate"
      type="date"
      placeholder="选择日期">
    </el-date-picker>
      </el-form-item>
...
<script>
import { createEquipmentInstance, getEquipmentInstance, updateEquipmentInstance } from "@/api/equipmentInstance";
//默认信息
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          this.$confirm("是否提交数据", "提示", {
            ...
            if (this.isEdit) {
              updateEquipmentInstance(this.$route.query.id, this.equipmentInstance).then(
                response => {
